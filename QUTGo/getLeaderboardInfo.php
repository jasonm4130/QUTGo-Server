...
function getFriends($connect, $userid){
	...
	*/
	$sql = "SELECT user_id FROM ((SELECT user_one AS user_id FROM relationship WHERE user_two = '$userid')UNION(SELECT user_two AS user_id FROM relationship WHERE user_one = '$userid')UNION(SELECT user_id FROM user WHERE user_id = '$userid'))derived_table";
